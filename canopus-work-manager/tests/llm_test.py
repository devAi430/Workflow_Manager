...
from canopusinfosystems.llm import CONTEXT_WINDOW_USAGE_RATIO, LLM
...
def test_validate_call_params_supported():
    ...
    with patch("canopusinfosystems.llm.supports_response_schema", return_value=True):
        llm = LLM(
...
def test_validate_call_params_not_supported():
    ...
    with patch("canopusinfosystems.llm.supports_response_schema", return_value=False):
...
def test_context_window_validation():
    ...
    with pytest.raises(ValueError) as excinfo:
        with patch.dict(
            "canopusinfosystems.llm.LLM_CONTEXT_WINDOW_SIZES",
...
def mock_emit() -> MagicMock:
    from canopusinfosystems.utilities.events.crewai_event_bus import CrewAIEventsBus
