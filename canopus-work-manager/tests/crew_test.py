...
from canopusinfosystems.agents import CacheHandler
from canopusinfosystems.crew import Crew
from canopusinfosystems.crews.crew_output import CrewOutput
from canopusinfosystems.flow import Flow, start
from canopusinfosystems.knowledge.knowledge import Knowledge
from canopusinfosystems.knowledge.source.string_knowledge_source import StringKnowledgeSource
from canopusinfosystems.llm import LLM
from canopusinfosystems.memory.contextual.contextual_memory import ContextualMemory
from canopusinfosystems.memory.long_term.long_term_memory import LongTermMemory
from canopusinfosystems.memory.short_term.short_term_memory import ShortTermMemory
from canopusinfosystems.process import Process
from canopusinfosystems.task import Task
from canopusinfosystems.tasks.conditional_task import ConditionalTask
from canopusinfosystems.tasks.output_format import OutputFormat
from canopusinfosystems.tasks.task_output import TaskOutput
from canopusinfosystems.types.usage_metrics import UsageMetrics
from canopusinfosystems.utilities import Logger
from canopusinfosystems.utilities.events import (
    CrewTrainCompletedEvent,
    CrewTrainStartedEvent,
    crewai_event_bus,
)
from canopusinfosystems.utilities.events.crew_events import (
    CrewTestCompletedEvent,
    CrewTestStartedEvent,
)
from canopusinfosystems.utilities.events.event_listener import EventListener
from canopusinfosystems.utilities.rpm_controller import RPMController
from canopusinfosystems.utilities.task_output_storage_handler import TaskOutputStorageHandler

from canopusinfosystems.utilities.events.memory_events import (
...
def test_crew_with_delegating_agents_should_not_override_task_tools(ceo, writer):
    ...
    from canopusinfosystems.tools import BaseTool
...
def test_cache_hitting_between_agents(researcher, writer, ceo):
    ...
    from canopusinfosystems.tools import tool
...
def test_crew_function_calling_llm():
    from canopusinfosystems import LLM
    from canopusinfosystems.tools import tool
...
def test_warning_long_term_memory_without_entity_memory():
    ...
    with (
        patch("canopusinfosystems.utilities.printer.Printer.print") as mock_print,
        patch(
            "canopusinfosystems.memory.long_term.long_term_memory.LongTermMemory.save"
        ) as save_memory,
    ):
        crew.kickoff()
...
@patch("canopusinfosystems.crew.Crew.kickoff")
@patch("canopusinfosystems.crew.CrewTrainingHandler")
@patch("canopusinfosystems.crew.TaskEvaluator")
@patch("canopusinfosystems.crew.Crew.copy")
...
def test_replay_with_context():
    ...
    with patch(
        "canopusinfosystems.utilities.task_output_storage_handler.TaskOutputStorageHandler.load",
        return_value=[
            {
...
def test_replay_with_context_set_to_nullable():
    ...
    with patch("canopusinfosystems.task.Task.execute_sync") as mock_execute_task:
        mock_execute_task.return_value = TaskOutput(
            description="Test Task Output",
            raw="test raw output",
            agent="test_agent",
...
@mock.patch("canopusinfosystems.crew.CrewEvaluator")
@mock.patch("canopusinfosystems.crew.Crew.copy")
@mock.patch("canopusinfosystems.crew.Crew.kickoff")
...
def test_multimodal_flag_adds_multimodal_tools():
    ...
    """
    from canopusinfosystems.tools.agent_tools.add_image_tool import AddImageTool
...
def test_before_kickoff_callback():
    from canopusinfosystems.project import CrewBase
    ...
    class TestCrewClass:
        ...
        from canopusinfosystems.project import CrewBase, agent, before_kickoff, crew, task
...
def test_before_kickoff_without_inputs():
    from canopusinfosystems.project import CrewBase, agent, before_kickoff, task

    @CrewBase
    class TestCrewClass:
        from canopusinfosystems.project import crew
