...
from canopusinfosystems import LLM, Agent
...
from canopusinfosystems.utilities.events.agent_events import LiteAgentExecutionStartedEvent
from canopusinfosystems.utilities.events.tool_usage_events import ToolUsageStartedEvent
...
def test_lite_agent_created_with_correct_parameters(monkeypatch, verbose):
    ...
    monkeypatch.setattr("canopusinfosystems.agent.LiteAgent", MockLiteAgent)
...
def test_guardrail_is_called_using_string():
    ...
    from canopusinfosystems.utilities.events import LLMGuardrailCompletedEvent, LLMGuardrailStartedEvent
    with crewai_event_bus.scoped_handlers():
...
def test_lite_agent_with_invalid_llm():
    ...
    with patch('canopusinfosystems.lite_agent.create_llm', return_value=None):
