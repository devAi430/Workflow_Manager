...
from canopusinfosystems.cli.deploy.main import DeployCommand
from canopusinfosystems.cli.utils import parse_toml


class TestDeployCommand(unittest.TestCase):
    @patch("canopusinfosystems.cli.command.get_auth_token")
    @patch("canopusinfosystems.cli.deploy.main.get_project_name")
    @patch("canopusinfosystems.cli.command.PlusAPI")
    ...
    @patch("canopusinfosystems.cli.deploy.main.DeployCommand._display_deployment_info")
    ...
    @patch("canopusinfosystems.cli.deploy.main.fetch_and_json_env_file")
    @patch("canopusinfosystems.cli.deploy.main.git.Repository.origin_url")
    ...
    def test_parse_toml_python_311_plus(self):
        ...
        python = "^3.11"
        canopusinfosystems = { extras = ["tools"], version = ">=0.51.0,<1.0.0" }
        """
    ...
    @patch(
        ...
        dependencies = ["canopusinfosystems"]
        """,
    )
    def test_get_project_name_python_310(self, mock_open):
        from canopusinfosystems.cli.utils import get_project_name
    ...
    def test_get_crewai_version(self):
        from canopusinfosystems.cli.version import get_crewai_version
