...
class TestPlusAPI(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertTrue("Canopus Infosystems-CLI/" in self.api.headers["User-Agent"])
        self.assertTrue(self.api.headers["X-Crewai-Version"])

    @patch("canopusinfosystems.cli.plus_api.PlusAPI._make_request")
    ...
    def assert_request_with_org_id(self, mock_make_request, method: str, endpoint: str, **kwargs):
        mock_make_request.assert_called_once_with(
            method, f"https://app.canopusinfosystems.com{endpoint}", headers={'Authorization': ANY, 'Content-Type': ANY, 'User-Agent':  ANY, 'X-Crewai-Version': ANY, 'X-Crewai-Organization-Id': self.org_uuid}, **kwargs
        )

    @patch("canopusinfosystems.cli.plus_api.Settings")
    ...
    @patch("canopusinfosystems.cli.plus_api.requests.Session")
    ...
    @patch.dict(os.environ, {"CANOPUSINFOSYSTEMS_BASE_URL": "https://custom-url.com/api"})
