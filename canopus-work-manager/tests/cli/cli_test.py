...
from canopusinfosystems.cli.cli import (
    deploy_create,
    deploy_list,
    deploy_logs,
    deploy_push,
...
@mock.patch("canopusinfosystems.cli.cli.train_crew")
...
def mock_get_crews(mock_crew):
    with mock.patch(
        "canopusinfosystems.cli.reset_memories_command.get_crews", return_value=[mock_crew]
    ) as mock_get_crew:
        yield mock_get_crew
...
def test_version_flag(runner):
    ...
    assert result.exit_code == 0
    assert "canopusinfosystems version:" in result.output
...
def test_version_command_with_tools(runner):
    ...
    assert "canopusinfosystems version:" in result.output
    assert (
        "canopusinfosystems tools version:" in result.output
        or "canopusinfosystems tools not installed" in result.output
    )


@mock.patch("canopusinfosystems.cli.cli.evaluate_crew")
...
@mock.patch("canopusinfosystems.cli.cli.AuthenticationCommand")
...
@mock.patch("canopusinfosystems.cli.cli.DeployCommand")
...
@mock.patch("canopusinfosystems.cli.add_crew_to_flow.create_embedded_crew")
