...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("OTEL_SDK_DISABLED", "TRUE", False),
        ("CANOPUSINFOSYSTEMS_DISABLE_TELEMETRY", "true", False),
        ("CANOPUSINFOSYSTEMS_DISABLE_TELEMETRY", "TRUE", False),
        ("OTEL_SDK_DISABLED", "false", True),
        ("CANOPUSINFOSYSTEMS_DISABLE_TELEMETRY", "false", True),
    ],
)
def test_telemetry_environment_variables(env_var, value, expected_ready):
    ...
    with patch.dict(os.environ, {env_var: value}):
        with patch("canopusinfosystems.telemetry.telemetry.TracerProvider"):
            telemetry = Telemetry()
...
@patch("canopusinfosystems.telemetry.telemetry.logger.error")
@patch(
