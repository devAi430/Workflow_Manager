...
from canopusinfosystems.knowledge.source.crew_docling_source import CrewDoclingSource
from canopusinfosystems.knowledge.source.csv_knowledge_source import CSVKnowledgeSource
from canopusinfosystems.knowledge.source.excel_knowledge_source import ExcelKnowledgeSource
from canopusinfosystems.knowledge.source.json_knowledge_source import JSONKnowledgeSource
from canopusinfosystems.knowledge.source.pdf_knowledge_source import PDFKnowledgeSource
from canopusinfosystems.knowledge.source.string_knowledge_source import StringKnowledgeSource
from canopusinfosystems.knowledge.source.text_file_knowledge_source import TextFileKnowledgeSource


@pytest.fixture(autouse=True)
def mock_vector_db():
    """Mock vector database operations."""
    with patch("canopusinfosystems.knowledge.storage.knowledge_storage.KnowledgeStorage") as mock:
...
def test_pdf_knowledge_source(mock_vector_db):
    ...
    mock_vector_db.query.return_value = [
        {"context": "canopusinfosystems create crew latest-ai-development", "score": 0.9}
    ]
    ...
    # Assert that the correct information is retrieved
    assert any(
        "canopusinfosystems create crew latest-ai-development" in result["context"].lower()
        for result in results
    )
    mock_vector_db.query.assert_called_once()
