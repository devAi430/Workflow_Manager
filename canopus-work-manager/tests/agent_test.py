...
from canopusinfosystems import Agent, Crew, Task
from canopusinfosystems.agents.cache import CacheHandler
from canopusinfosystems.agents.crew_agent_executor import AgentFinish, CrewAgentExecutor
from canopusinfosystems.knowledge.knowledge import Knowledge
from canopusinfosystems.knowledge.knowledge_config import KnowledgeConfig
...
from canopusinfosystems.tools.tool_calling import InstructorToolCalling
from canopusinfosystems.tools.tool_usage import ToolUsage
from canopusinfosystems.utilities import RPMController
from canopusinfosystems.utilities.errors import AgentRepositoryError
from canopusinfosystems.utilities.events import crewai_event_bus
from canopusinfosystems.utilities.events.tool_usage_events import ToolUsageFinishedEvent
...
@patch("canopusinfosystems.agent.CrewTrainingHandler")
...
def test_handle_context_length_exceeds_limit():
    ...
    from canopusinfosystems.utilities.agent_utils import handle_context_length
    from canopusinfosystems.utilities.i18n import I18N
    from canopusinfosystems.utilities.printer import Printer
    ...
    with patch("canopusinfosystems.utilities.agent_utils.summarize_messages") as mock_summarize:
...
def test_handle_context_length_exceeds_limit_cli_no():
    ...
    with patch.object(
        ...
        with patch(
            "canopusinfosystems.utilities.agent_utils.handle_context_length"
...
def test_agent_with_knowledge_sources():
    ...
    with patch("canopusinfosystems.knowledge") as MockKnowledge:
...
def test_agent_with_knowledge_sources_with_query_limit_and_score_threshold():
    ...
    with patch(
        "canopusinfosystems.knowledge.storage.knowledge_storage.KnowledgeStorage"
    ) as MockKnowledge:
        mock_knowledge_instance = MockKnowledge.return_value
        mock_knowledge_instance.sources = [string_source]
...
def test_agent_with_knowledge_sources_works_with_copy():
    ...
    with patch(
        "canopusinfosystems.knowledge.source.base_knowledge_source.BaseKnowledgeSource",
        autospec=True,
...
def test_crew_agent_executor_litellm_auth_error():
    ...
    from canopusinfosystems.agents.tools_handler import ToolsHandler
    from canopusinfosystems.utilities import Printer
...
def mock_get_auth_token():
    with patch(
        "canopusinfosystems.cli.authentication.token.get_auth_token", return_value="test_token"
    ):
        yield


@patch("canopusinfosystems.cli.plus_api.PlusAPI.get_agent")
...
@patch("canopusinfosystems.cli.plus_api.PlusAPI.get_agent")
@patch("canopusinfosystems.utilities.agent_utils.Settings")
@patch("canopusinfosystems.utilities.agent_utils.console")
...
def test_agent_from_repository_without_org_set(
    ...
    mock_console.print.assert_any_call(
        "No organization currently set. We recommend setting one before using: `canopusinfosystems org switch <org_id>` command.",
        style="yellow",
    )
