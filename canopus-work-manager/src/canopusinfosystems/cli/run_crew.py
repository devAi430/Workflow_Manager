...
def run_crew() -> None:
    ...
    if pyproject_data.get("tool", {}).get("poetry") and (
        ...
        click.secho(
            ...
            f"Please run `canopusinfosystems update` to update your pyproject.toml to use uv.",
            fg="red",
        )

    # Determine crew type
    is_flow = pyproject_data.get("tool", {}).get("canopusinfosystems", {}).get("type") == "flow"
...
def handle_error(error: subprocess.CalledProcessError, crew_type: CrewType) -> None:
    ...
    if pyproject_data.get("tool", {}).get("poetry"):
        click.secho(
            ...
            "please run `canopusinfosystems update` to update your pyproject.toml to use uv.",
            fg="yellow",
        )
