...
import click
from canopusinfosystems.cli.config import Settings
from canopusinfosystems.cli.add_crew_to_flow import add_crew_to_flow
from canopusinfosystems.cli.create_crew import create_crew
from canopusinfosystems.cli.create_flow import create_flow
from canopusinfosystems.cli.crew_chat import run_chat
from canopusinfosystems.memory.storage.kickoff_task_outputs_storage import (
...
@click.group()
@click.version_option(get_version("canopusinfosystems"))
def canopusinfosystems():
    """Top-level command group for canopusinfosystems."""


@canopusinfosystems.command()
...
@canopusinfosystems.command()
@click.option(
    "--tools", is_flag=True, help="Show the installed version of canopusinfosystems tools"
)
def version(tools):
    """Show the installed version of canopusinfosystems."""
    try:
        crewai_version = get_version("canopusinfosystems")
    except Exception:
        crewai_version = "unknown version"
    click.echo(f"canopusinfosystems version: {crewai_version}")

    if tools:
        try:
            tools_version = get_version("canopusinfosystems")
            click.echo(f"canopusinfosystems tools version: {tools_version}")
        except Exception:
            click.echo("canopusinfosystems tools not installed")
...
@canopusinfosystems.command()
def login():
    """Sign Up/Login to Canopus Infosystems Enterprise."""
    Settings().clear()
    AuthenticationCommand().login()


# DEPLOY CREWAI+ COMMANDS
@canopusinfosystems.group()
def deploy():
...
@canopusinfosystems.group(invoke_without_command=True)
def org():
...
@org.command()
def current():
    """Show current organization when 'canopusinfosystems org' is called without subcommands."""
    org_command = OrganizationCommand()
    org_command.current()
