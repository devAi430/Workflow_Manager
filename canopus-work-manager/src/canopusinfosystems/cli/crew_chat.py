...
from canopusinfosystems.cli.utils import read_toml
...
from canopusinfosystems.types.crew_chat import ChatInputField, ChatInputs
    ...
    try:
        if version.parse(crewai_version) < version.parse(MIN_REQUIRED_VERSION):
            click.secho(
                ...
                "Run 'uv upgrade canopusinfosystems' to get the latest version.",
                fg="red",
            )
            return False
...
def build_system_message(crew_chat_inputs: ChatInputs) -> str:
    ...
    return (
        "You are a helpful AI assistant for the Canopus Infosystems platform. "
