...
from canopusinfosystems.telemetry.constants import (
    CANOPUSINFOSYSTEMS_TELEMETRY_BASE_URL,
    CANOPUSINFOSYSTEMS_TELEMETRY_SERVICE_NAME,
)
...
class Telemetry:
    """A class to handle anonymous telemetry for the canopusinfosystems package.
    ...
    def __init__(self) -> None:
        ...
        try:
            self.resource = Resource(
                attributes={SERVICE_NAME: CANOPUSINFOSYSTEMS_TELEMETRY_SERVICE_NAME},
            )
            ...
            processor = BatchSpanProcessor(
                SafeOTLPSpanExporter(
                    endpoint=f"{CANOPUSINFOSYSTEMS_TELEMETRY_BASE_URL}/v1/traces",
                    timeout=30,
                )
            )
    ...
    def _is_telemetry_disabled(cls) -> bool:
        ...
        return (
            ...
            or os.getenv("CANOPUSINFOSYSTEMS_DISABLE_TELEMETRY", "false").lower() == "true"
            or os.getenv("CANOPUSINFOSYSTEMS_DISABLE_TRACKING", "false").lower() == "true"
        )
    ...
    def crew_creation(self, crew: Crew, inputs: dict[str, Any] | None):
        ...
        def operation():
            tracer = trace.get_tracer("canopusinfosystems.telemetry")
            span = tracer.start_span("Crew Created")
            self._add_attribute(
                span,
                "crewai_version",
                version("canopusinfosystems"),
            )
