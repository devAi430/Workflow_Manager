...
from canopusinfosystems.tools import BaseTool as CrewAITool
...
from canopusinfosystems.utilities.exceptions.context_window_exceeding_exception import (
...
def _print_current_organization():
    ...
    if settings.org_uuid:
        ...
        console.print("No organization currently set. We recommend setting one before using: `canopusinfosystems org switch <org_id>` command.", style="yellow")
...
def load_agent_from_repository(from_repository: str) -> Dict[str, Any]:
    ...
    if from_repository:
        ...
        from canopusinfosystems.cli.authentication.token import get_auth_token
        from canopusinfosystems.cli.plus_api import PlusAPI
        ...
        if response.status_code == 404:
            raise AgentRepositoryError(
                ...
                f"\nIf you are using the wrong organization, switch to the correct one using `canopusinfosystems org switch <org_id>` command.",
            )
